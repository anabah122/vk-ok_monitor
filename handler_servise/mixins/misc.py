...
class MiscMixin:
    ...
    def on_group_officers_edit(self, o):
    ...
    def on_group_change_settings(self, o):
        user    = o.get("user_id")
    ...
    def on_group_change_photo(self, o):
        user = o.get("user_id")
        print(f"  🖼️  Фото сообщества изменено пользователем {user}")

    def on_vkpay_transaction(self, o):
    ...
    def on_donut_subscription_create(self, o):
        user   = o.get("user_id")
    ...
    def on_donut_subscription_prolonged(self, o):
        user   = o.get("user_id")
        amount = o.get("amount", 0)
    ...
    def on_donut_subscription_expired(self, o):
        user = o.get("user_id")
        print(f"  🍩 Подписка Donut истекла: {user}")
        db.insert("donut_subscription", {"event_type": "expired", "user_id": user})

    def on_donut_subscription_cancelled(self, o):
        user = o.get("user_id")
        print(f"  🍩 Подписка Donut отменена: {user}")
        db.insert("donut_subscription", {"event_type": "cancelled", "user_id": user})

    def on_donut_subscription_price_changed(self, o):
        user       = o.get("user_id")
    ...
    def on_donut_money_withdraw(self, o):
        amount = o.get("amount", 0)
    ...
    def on_donut_money_withdraw_error(self, o):
        reason = o.get("reason", "")
        print(f"  🍩❌ Ошибка вывода Donut: {reason}")
        db.insert("donut_money_withdraw", {"event_type": "withdraw_error", "reason": reason})
